...
// Select all / deselect all (checkboxes in new version)
...
// Fallback: if still buttons (old behavior)
...
// Export selected button (new toolbar)
...
function showPreviewPanel(points, projection, endpoint) {
  pendingPreviewData = { points, endpoint };
...
document.getElementById('previewCancelBtn').addEventListener('click', () => {
  ...
  // Clean up file input
  document.getElementById('importShpAutoFile').value = '';
  document.getElementById('importExcelLambertFile').value = '';
  document.getElementById('importExcelWgs84File').value = '';
  document.getElementById('importKmlFile').value = '';
});

document.getElementById('previewValidateBtn').addEventListener('click', async () => {
  if (!pendingPreviewData) return;
  const { endpoint } = pendingPreviewData;
  const status = document.getElementById('previewStatus');
  status.textContent = 'Import en cours...';
  status.style.color = '#555';

  try {
    const res = await fetch(endpoint, { method: 'POST', body: pendingPreviewData.formData });
    const data = await res.json();
    if (res.ok) {
...
document.getElementById('importShpAutoFile').addEventListener('change', async (e) => {
  ...
  if (!file) return;
  if (!confirm(`Importer le fichier "${file.name}" (SHP ZIP avec auto-détection) ?`)) {
    e.target.value = '';
    return;
  }
  ...
  try {
    ...
    if (res.ok && data.preview) {
      status.textContent = '';
      pendingPreviewData = { points: data.preview.points, endpoint: '/api/import-shp-auto/confirm', formData };
      showPreviewPanel(data.preview.points, data.projection || 'Auto-détectée', '/api/import-shp-auto/confirm');
    } else {
      status.textContent = 'Erreur : ' + (data.error || 'import échoué');
    }
  } catch (err) {
...
document.getElementById('importExcelLambertFile').addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (res.ok && data.preview) {
      status.textContent = '';
      pendingPreviewData = { points: data.preview, endpoint: '/api/import', formData };
      showPreviewPanel(data.preview, 'Lambert (X/Y)', '/api/import');
    } else {
      status.textContent = 'Erreur : ' + (data.error || 'import échoué');
    }
  } catch (err) {
...
document.getElementById('importExcelWgs84File').addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (res.ok && data.preview) {
      status.textContent = '';
      pendingPreviewData = { points: data.preview, endpoint: '/api/import-wgs84', formData };
      showPreviewPanel(data.preview, 'WGS84 (Lat/Lng)', '/api/import-wgs84');
    } else {
      status.textContent = 'Erreur : ' + (data.error || 'import échoué');
    }
  } catch (err) {
...
document.getElementById('visualizeSectorsBtn').addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    sectorPreviewLayers.forEach((l) => sectorMapInstance.removeLayer(l));
    sectorPreviewLayers = [];
...
let pendingSectorPreviewData = null;

function showSectorPreviewPanel(sectors, projection, endpoint) {
  pendingSectorPreviewData = { sectors, endpoint };
  ...
  setTimeout(() => {
    ...
    if (sectorPreviewMapInstance) {
      sectorPreviewMapInstance.remove();
      sectorPreviewMapInstance = null;
    }
...
document.getElementById('sectorPreviewCancelBtn').addEventListener('click', () => {
  ...
  document.getElementById('importSectorShpFile').value = '';
  document.getElementById('importSectorExcelFile').value = '';
});

document.getElementById('sectorPreviewValidateBtn').addEventListener('click', async () => {
  if (!pendingSectorPreviewData) return;
  const { endpoint, formData } = pendingSectorPreviewData;
...
document.getElementById('importSectorShpFile').addEventListener('change', async (e) => {
  const file = e.target.files[0];
  if (!file) return;
  if (!confirm(`Importer le fichier "${file.name}" (SHP ZIP secteurs, auto-détection) ?`)) {
    e.target.value = '';
    return;
  }
  const formData = new FormData();
  ...
  try {
    ...
    const data = await res.json();
    if (res.ok && data.preview) {
      pendingSectorPreviewData = { sectors: data.preview, endpoint: '/api/sectors/import-shp', formData };
      showSectorPreviewPanel(data.preview, data.projection || 'Auto-détectée', '/api/sectors/import-shp');
    } else {
...
document.getElementById('importSectorExcelFile').addEventListener('change', async (e) => {
  ...
  try {
    ...
    const data = await res.json();
    if (res.ok && data.preview) {
      pendingSectorPreviewData = { sectors: data.preview, endpoint: '/api/sectors/import', formData };
      showSectorPreviewPanel(data.preview, 'Excel', '/api/sectors/import');
    } else {
      alert('Erreur : ' + (data.error || 'import échoué'));
    }
  } catch (err) {
