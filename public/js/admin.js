...
function renderTable() {
  ...
  if (pagePoints.length === 0) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="12" class="empty-state">Aucun point trouvé</td></tr>';
  } else {
...
// Action buttons: Point / Secteur (comme le site Next.js original)
// NOTE: Les boutons d'action Point/Secteur sont maintenant les onglets directement
// L'utilisateur clique sur l'onglet puis sur "+ Ajouter" pour créer
...
function renderSectorsTable() {
  ...
  // Show/hide empty state vs table (comme le site original)
  ...
  if (pageSectors.length === 0) {
    // Don't show table empty row when using the nice empty state
  } else {
...
function updateSectorSelectionUI() {
  const info = document.getElementById('sectorSelectionInfo');
  if (info) info.textContent = `${selectedSectorIds.size} secteur(s)`;
  const deleteCount = document.getElementById('sectorDeleteCount');
  if (deleteCount) deleteCount.textContent = selectedSectorIds.size;
}

// Sector select all / deselect all
const sectorSelectAllCb = document.getElementById('sectorSelectAllBtn');
const sectorDeselectAllCb = document.getElementById('sectorDeselectAllBtn');
if (sectorSelectAllCb) {
  sectorSelectAllCb.addEventListener('change', () => {
    if (sectorSelectAllCb.checked) filteredSectors.forEach((s) => selectedSectorIds.add(s.id));
    updateSectorSelectionUI(); renderSectorsTable();
  });
}
if (sectorDeselectAllCb) {
  sectorDeselectAllCb.addEventListener('change', () => {
    if (sectorDeselectAllCb.checked) { selectedSectorIds.clear(); updateSectorSelectionUI(); renderSectorsTable(); }
  });
}
...
runImport(document.getElementById('importSectorShpFile'), '/api/sectors/import-shp', 'secteurs SHP');
runImport(document.getElementById('importSectorKmlFile'), '/api/sectors/import-kml', 'secteurs KML');
