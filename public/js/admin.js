...
document.querySelectorAll('.admin-tab').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('tabPoints').classList.toggle('hidden', tab !== 'interventions');
    ...
    if (tab === 'map') initAdminFullMap();
  });
});

function initAdminFullMap() {
  if (adminFullMap) return;
  adminFullMap = L.map('adminFullMap').setView([35.5785, -5.3684], 9);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; OpenStreetMap contributors',
    maxZoom: 19,
  }).addTo(adminFullMap);
  setTimeout(() => adminFullMap.invalidateSize(), 200);
  allPoints.filter(p => p.lat != null && p.lng != null).forEach(p => {
    L.marker([p.lat, p.lng]).addTo(adminFullMap)
      .bindPopup(`<b>IG:</b> ${p.ig || '—'}<br><b>Adresse:</b> ${p.adresse || '—'}`);
  });
}
