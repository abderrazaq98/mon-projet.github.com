...
// ---- Import SHP Auto-detect (preview) ----
router.post('/import-shp-auto', requireAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
  ...
  try {
    ...
    let result = await source.read();
    while (!result.done && previewPoints.length < 50) {
      total++;
      const feature = result.value;
      const coords = feature.geometry && feature.geometry.coordinates;
      if (!coords) { result = await source.read(); continue; }
      ...
      if (projection === 'Lambert') {
        // Coordinates are Lambert X/Y, need to swap and convert
        const x = lngRaw;
        const y = latRaw;
        const conv = lambertToWGS84(x, y);
        previewPoints.push({ ig, adresse, numero_batiment: numeroBatiment, secteur_nom: secteurNom, secteur_numero: secteurNumero, x, y, lat: conv.lat, lng: conv.lng });
      } else {
        // WGS84: lng/lat
        const lat = latRaw;
        const lng = lngRaw;
        const conv = wgs84ToLambert(lat, lng);
        previewPoints.push({ ig, adresse, numero_batiment: numeroBatiment, secteur_nom: secteurNom, secteur_numero: secteurNumero, x: conv.x, y: conv.y, lat, lng });
      }
      result = await source.read();
    }

    // Count remaining for total
    while (!result.done) {
      total++;
      result = await source.read();
    }
...
// ---- Import SHP Auto-detect (confirm) ----
router.post('/import-shp-auto/confirm', requireAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
...
// ---- Import Excel Lambert (full preview for all rows) ----
router.post('/import-preview', requireAuth, upload.single('file'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (x != null && y != null && !Number.isNaN(x) && !Number.isNaN(y)) {
      const conv = lambertToWGS84(x, y);
      previewPoints.push({
        ig: ig ? String(ig) : null,
        adresse: adresse ? String(adresse) : null,
        numero_batiment: numeroBatiment != null ? String(numeroBatiment) : null,
        secteur_nom: secteurNom != null ? String(secteurNom) : null,
        secteur_numero: secteurNumero != null ? String(secteurNumero) : null,
        x, y, lat: conv.lat, lng: conv.lng,
      });
    } else {
      errors.push({ row: i + 1, reason: 'Coordonnées X/Y manquantes ou invalides' });
    }
...
// ---- Import Excel WGS84 (preview) ----
router.post('/import-wgs84-preview', requireAuth, upload.single('file'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const lat = map['latitude'] ?? map['lat'] ?? map['y'] != null ? parseFloat(map['latitude'] || map['lat'] || map['y']) : null;
    const lng = map['longitude'] ?? map['lng'] ?? map['long'] ?? map['x'] != null ? parseFloat(map['longitude'] || map['lng'] || map['long'] || map['x']) : null;

    // Auto-detect: if lat is between -90/90 and lng is between -180/180, it's WGS84
    if (lat != null && lng != null && !Number.isNaN(lat) && !Number.isNaN(lng)) {
      const isWGS84 = (lat >= -90 && lat <= 90) && (lng >= -180 && lng <= 180);
      if (isWGS84) {
        const conv = wgs84ToLambert(lat, lng);
        previewPoints.push({
          ig: ig ? String(ig) : null,
          ...
          secteur_nom: secteurNom != null ? String(secteurNom) : null,
          secteur_numero: secteurNumero != null ? String(secteurNumero) : null,
          x: conv.x, y: conv.y, lat, lng,
        });
      } else {
        // Treat as Lambert coordinates
        const conv = lambertToWGS84(lng, lat);
        previewPoints.push({
          ig: ig ? String(ig) : null,
          adresse: adresse ? String(adresse) : null,
          numero_batiment: numeroBatiment != null ? String(numeroBatiment) : null,
          secteur_nom: secteurNom != null ? String(secteurNom) : null,
          secteur_numero: secteurNumero != null ? String(secteurNumero) : null,
          x: lng, y: lat, lat: conv.lat, lng: conv.lng,
        });
      }
    } else {
      errors.push({ row: i + 1, reason: 'Coordonnées Lat/Lng manquantes ou invalides' });
    }
  }
...
router.post('/import-wgs84', requireAuth, upload.single('file'), (req, res) => {
  ...
  try {
    for (const row of rows) {
      ...
      const secteurNumero = map['numero secteur'] ?? map['num secteur'] ?? map['n secteur']
        ?? map['numero du secteur'] ?? null;
      const lat = map['latitude'] ?? map['lat'] ?? null;
      const lng = map['longitude'] ?? map['lng'] ?? map['long'] ?? null;
      ...
      const conv = wgs84ToLambert(latVal, lngVal);
      insert.run(
        ig ? String(ig) : null,
        adresse ? String(adresse) : null,
        numeroBatiment != null ? String(numeroBatiment) : null,
