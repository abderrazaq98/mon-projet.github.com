...
// ---- Extract geometry from SHP ZIP (for sector modal import) ----
router.post('/sectors/extract-geometry', requireAuth, upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
  ...
  try {
    ...
    return res.status(500).json({ error: 'Dépendances manquantes: npm install shapefile adm-zip' });
  }
  ...
  try {
    ...
    while (!result.done) {
      if (result.value.geometry) {
        allFeatures.push(result.value.geometry);
      }
      result = await source.read();
    }

    if (allFeatures.length === 0) {
      return res.status(400).json({ error: 'Aucune géométrie trouvée dans le fichier SHP' });
    }

    // If multiple features, merge into a FeatureCollection
    let geometry;
    if (allFeatures.length === 1) {
      const converted = convertGeoCoords(allFeatures[0], projection);
      geometry = JSON.stringify(converted);
    } else {
      // Merge all features into a GeometryCollection or FeatureCollection
      const converted = allFeatures.map(g => convertGeoCoords(g, projection));
      const featureCollection = {
        type: 'FeatureCollection',
        features: converted.map(g => ({ type: 'Feature', geometry: g, properties: {} }))
      };
      geometry = JSON.stringify(featureCollection);
    }

    res.json({ geometry, projection, featureCount: allFeatures.length });
  } catch (e) {
    res.status(400).json({ error: 'Fichier SHP illisible: ' + e.message });
  }
});
